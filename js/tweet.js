...
class Tweet {
    ...
    get activityType() {
        ...
        // Normalize synonyms
        if (/(\brun|jog(ging)?\b)/.test(phrase))
            return 'running';
        if (/(\bwalk(ing)?\b)/.test(phrase))
            return 'walking';
        if (/(\b(bike|biking|cycling)\b|\bbike ride\b|\bride\b)/.test(phrase))
            return 'biking';
        if (/(\bhike(ing)?\b)/.test(phrase))
            return 'hiking';
        if (/(\bswim(ming)?\b)/.test(phrase))
            return 'swimming';
        if (/(\belliptical\b)/.test(phrase))
            return 'elliptical';
        if (/(\brow(ing)?\b)/.test(phrase))
            return 'rowing';
        if (/(\bski(ing)?|snowboard(ing)?|xc ski\b)/.test(phrase))
            return 'skiing';
        if (/(\byoga\b)/.test(phrase))
            return 'yoga';
        if (/(\bkayak(ing)?\b)/.test(phrase))
            return 'kayaking';
        if (/(\bskate(ing)?\b)/.test(phrase))
            return 'skating';
        if (/(\btreadmill\b)/.test(phrase))
            return 'treadmill';
        return 'unknown';
    }
    get distance() {
    ...
    extractUserWrittenText(raw) {
        ...
        const wordCount = (s) => (s.match(/\b[a-zA-Z]{2,}\b/g) || []).length;
        const strongWord = (s) => /\b[a-zA-Z]{8,}\b/.test(s);
